...
class SubsidyLog(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    producer_address = db.Column(db.String(42), nullable=False)
    action = db.Column(db.String(50), nullable=False) # e.g., 'register', 'set_milestone', 'release'
    details = db.Column(db.JSON, nullable=True)
    tx_hash = db.Column(db.String(66), nullable=True)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)

    def __repr__(self):
        return f'<SubsidyLog {self.action} for {self.producer_address}>'
